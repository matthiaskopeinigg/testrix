...
import { TxBrandLogoComponent } from '../../chrome/tx-brand-logo/tx-brand-logo.component';
...
export class TxSettingsPopupComponent {
  ...
  readonly bridgeMeta = computed(() => {
    ...
    const versions = typeof bridge.getVersions === 'function' ? bridge.getVersions() : bridge.versions;
    return {
      platform: bridge.platform,
      app: versions.app,
      installedApp: versions.installedApp,
  ...
  protected readonly displayAppVersion = computed(() => {
    const meta = this.bridgeMeta();
    const installed = meta?.installedApp?.trim();
    if (installed) {
      return installed;
    }
    const app = meta?.app?.trim();
    return app || null;
  });
