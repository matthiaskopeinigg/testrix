...
export class TxCodeEditorComponent implements ControlValueAccessor, AfterViewInit {
  ...
  protected readonly templateVarsActive = computed(
    () =>
      !this.readOnly() &&
      this.variableCatalog().length > 0 &&
      this.overlaySyntaxHighlight(),
  );
  ...
  /**
   * Prefer the request section pane (split primary) so fill height stops above the
   * response panel, not at the full workspace tab edge.
   */
