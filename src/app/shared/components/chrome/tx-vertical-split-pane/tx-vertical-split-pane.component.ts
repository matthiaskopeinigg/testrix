...
/** Primary may collapse fully so the secondary panel can sit under fixed chrome (e.g. URL bar). */
const MIN_PRIMARY_HEIGHT = 0;
...
const SPLIT_HANDLE_HEIGHT_PX = 20;
      ...
      @if (secondaryVisible()) {
        ...
        @if (showPanel()) {
          ...
          >
            ...
            >
              <span class="tx-vertical-split-pane__grip" aria-hidden="true">
                <span class="tx-vertical-split-pane__grip-dot"></span>
                <span class="tx-vertical-split-pane__grip-dot"></span>
                <span class="tx-vertical-split-pane__grip-dot"></span>
              </span>
              <button
...
export class TxVerticalSplitPaneComponent {
  ...
  protected handlePointerDown(event: MouseEvent): void {
    ...
    const onMove = (e: MouseEvent): void => {
      ...
      const next = Math.min(
        this.maxSecondaryHeightPx(),
        Math.max(MIN_SECONDARY_HEIGHT, startHeight + delta),
      );
      lastHeight = next;
  ...
  private maxSecondaryHeightPx(): number {
    const hostHeight = this.hostEl.nativeElement.getBoundingClientRect().height;
    if (!Number.isFinite(hostHeight) || hostHeight <= 0) {
      return MIN_SECONDARY_HEIGHT;
    }
    return Math.max(
      MIN_SECONDARY_HEIGHT,
      Math.floor(hostHeight - this.handleHeightPx - MIN_PRIMARY_HEIGHT),
    );
  }
