...
export interface ElectronRendererBridge {
  ...
  /**
   * Live runtime versions from preload state.
   *
   * `versions` is cloned at `contextBridge` expose time; use this after updater
   * simulation or other main-process version changes.
   */
  getVersions: () => {
    readonly app: string;
    readonly installedApp: string;
