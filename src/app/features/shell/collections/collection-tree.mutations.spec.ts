...
describe('collection-tree.mutations', () => {
  ...
  it('preserves trailing spaces in request description while trimming subtitle', () => {
    const withDesc = setCollectionRequestDescription(COLLECTION_TREE_MOCK, 'req-login', 'Sign in ');
    const req = findRequest(withDesc ?? [], 'req-login');
    expect(req?.data?.description).toBe('Sign in ');
    expect(req?.subtitle).toBe('Sign in');
  });
