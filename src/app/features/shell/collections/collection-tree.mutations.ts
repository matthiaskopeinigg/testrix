...
/**
 * Persists description text as typed. Only whitespace-only input clears the field.
 * Tree subtitles still use a trimmed preview.
 */
function normalizeNodeDescription(description: string): {
  ...
  if (!trimmed) {
    return { stored: undefined, subtitle: undefined };
  }
  return { stored: description, subtitle: trimmed };
}
  ...
  const mapNodes = (list: readonly CollectionTreeNode[]): CollectionTreeNode[] =>
    list.map((node) => {
      if (node.id === folderId && (node.data?.kind === 'folder' || node.kind === 'folder')) {
        found = true;
        const { stored, subtitle } = normalizeNodeDescription(description);
        return {
          ...node,
          subtitle,
          data: {
