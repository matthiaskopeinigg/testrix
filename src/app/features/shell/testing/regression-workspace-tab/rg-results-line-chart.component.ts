...
@Component({
  selector: 'app-rg-results-line-chart',
  standalone: true,
  template: `
    <figure class="rg-line-chart">
      <figcaption class="rg-line-chart__caption">
        <span class="rg-line-chart__title">{{ title() }}</span>
        @if (stats()) {
          <span class="rg-line-chart__latest">{{ formatValue(stats()!.latest) }} {{ unit() }}</span>
        }
      </figcaption>

      @if (stats()) {
        <div class="rg-line-chart__legend" aria-label="Chart legend">
          <div class="rg-line-chart__legend-row">
            <span class="rg-line-chart__legend-item">
              <span class="rg-line-chart__legend-swatch" [style.background]="strokeColor()"></span>
              <span class="rg-line-chart__legend-label">{{ primaryLabel() }}</span>
            </span>
            <span class="rg-line-chart__legend-stat">Low {{ formatValue(stats()!.min) }}</span>
            <span class="rg-line-chart__legend-stat">Avg {{ formatValue(stats()!.avg) }}</span>
            <span class="rg-line-chart__legend-stat">High {{ formatValue(stats()!.max) }}</span>
          </div>
          @if (secondaryStats()) {
            <div class="rg-line-chart__legend-row">
              <span class="rg-line-chart__legend-item">
                <span
                  class="rg-line-chart__legend-swatch rg-line-chart__legend-swatch--dashed"
                  [style.--rg-swatch-color]="secondaryStrokeColor()"
                ></span>
                <span class="rg-line-chart__legend-label">{{ secondaryLabel() }}</span>
              </span>
              <span class="rg-line-chart__legend-stat">Low {{ formatValue(secondaryStats()!.min) }}</span>
              <span class="rg-line-chart__legend-stat">Avg {{ formatValue(secondaryStats()!.avg) }}</span>
              <span class="rg-line-chart__legend-stat">High {{ formatValue(secondaryStats()!.max) }}</span>
            </div>
          }
        </div>
      }

      <div class="rg-line-chart__plot">
        @if (yAxisTickLabels(); as ticks) {
          <div class="rg-line-chart__y-axis" aria-hidden="true">
            <span>{{ ticks[0] }}</span>
            <span>{{ ticks[1] }}</span>
            <span>{{ ticks[2] }}</span>
          </div>
        ...
        >
          ...
          @if (hasData()) {
            ...
            <polygon
              class="rg-line-chart__area"
              [attr.points]="areaPoints()"
              [attr.fill]="'url(#' + gradientId() + ')'"
            />
            <polyline
              class="rg-line-chart__line"
              [attr.points]="linePoints()"
              [attr.stroke]="strokeColor()"
            />
            @if (secondaryLinePoints()) {
              <polyline
                class="rg-line-chart__line rg-line-chart__line--secondary"
      ...
      @if (hasData()) {
        <div class="rg-line-chart__x-axis" aria-hidden="true">
          <span>0s</span>
  ...
  `,
  styleUrl: './rg-results-line-chart.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class RgResultsLineChartComponent {
...
function computeStats(values: readonly number[]): RgLineChartStats | null {
  if (values.length === 0) {
    return null;
  }
