      ...
      @for (run of runs(); track run.id) {
        ...
        >
          @if (compareSelection()?.a === run.id) {
            <span class="rg-run-timeline__slot rg-run-timeline__slot--a">A</span>
          } @else if (compareSelection()?.b === run.id) {
            <span class="rg-run-timeline__slot rg-run-timeline__slot--b">B</span>
          } @else if (pinnedId() === run.id) {
            <tx-icon name="star" [size]="12" class="rg-run-timeline__pin" aria-hidden="true" />
          } @else {
            <span
          ...
          <tx-tag [variant]="statusVariant(run.status)" casing="normal">
            {{ (run.summary?.passRatePercent ?? 0).toFixed(1) }}%
          </tx-tag>
        </button>
      }
    </div>
  `,
  styleUrl: './rg-results-run-timeline.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class RgResultsRunTimelineComponent {
  ...
  protected formatRunLabel(run: RegressionRun): string {
    ...
    return `${time} · ${passRate.toFixed(0)}% pass`;
  }

  protected chipTitle(run: RegressionRun): string {
    if (this.compareMode()) {
