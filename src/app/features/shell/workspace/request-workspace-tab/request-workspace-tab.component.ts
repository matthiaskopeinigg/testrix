...
export class RequestWorkspaceTabComponent {
  ...
  protected handleUrlPathChange(display: string): void {
    ...
    // One settings patch so urlDisplay does not briefly combine the new path with
    // stale query rows (e.g. clear-all flashing `?…` back into the bar).
    if (pathChanged || queryChanged) {
      this.patchSettings({
        ...(pathChanged ? { pathParams: pathSynced } : {}),
        ...(queryChanged ? { queryParams: parsed.queryParams } : {}),
      });
    }
  }
