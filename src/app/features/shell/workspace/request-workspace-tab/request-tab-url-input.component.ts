...
import { TxTooltipService } from '@app/shared/components/overlays/tx-tooltip/tx-tooltip.service';
import { TxVariableInputHighlightHtmlPipe } from '@app/shared/components/editors/tx-variable-input/tx-variable-input-highlight-html.pipe';
import { caretIndexFromClientX } from '@app/shared/components/editors/tx-variable-input/tx-variable-input-caret';
...
export class RequestTabUrlInputComponent implements ControlValueAccessor {
  ...
  writeValue(value: string | null): void {
    ...
    // Ignore parent rewrites while typing. Display URL is rebuilt from path + query
    // params (encoding / ordering), which would desync the mirror and feel like flicker.
    if (this.isFocused) {
      return;
    }
    this.value.set(next);
