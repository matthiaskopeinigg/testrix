 ...
 * Query keys/values are shown raw (not percent-encoded) so the bar matches the Params editor.
 * Persisted `url` on the node is the path only (no `?` query).
 * Use {@link buildRequestOutgoingUrl} when sending or generating wire-ready URLs.
 */
export function buildRequestDisplayUrl(
  path: string,
  queryParams: readonly HttpKeyValueRow[],
): string {
  return joinRequestUrl(path, queryParams, false);
}

/**
 * Builds a wire-ready request URL with percent-encoded query keys and values.
 */
export function buildRequestOutgoingUrl(
  path: string,
  queryParams: readonly HttpKeyValueRow[],
): string {
  return joinRequestUrl(path, queryParams, true);
}

function joinRequestUrl(
  path: string,
  queryParams: readonly HttpKeyValueRow[],
  encodeQuery: boolean,
): string {
