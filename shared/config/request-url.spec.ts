...
describe('buildRequestDisplayUrl', () => {
  it('appends enabled query params without encoding', () => {
  ...
  it('keeps colons and spaces raw in query values', () => {
    const url = buildRequestDisplayUrl('https://example.com/x', [
      createHttpKeyValueRow({ key: 'sadumym', value: ':asdasdfasdf', enabled: true }),
    ]);
    expect(url).toBe('https://example.com/x?sadumym=:asdasdfasdf');
  });
});

describe('buildRequestOutgoingUrl', () => {
  it('percent-encodes query keys and values for the wire', () => {
    const url = buildRequestOutgoingUrl('https://example.com/x', [
      createHttpKeyValueRow({ key: 'sadumym', value: ':asdasdfasdf', enabled: true }),
    ]);
    expect(url).toBe('https://example.com/x?sadumym=%3Aasdasdfasdf');
  });
});
