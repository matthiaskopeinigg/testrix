...
export const HELP_WIKI_GROUPS: readonly HelpWikiGroup[] = [
  ...
  helpWikiGroupSchema.parse({ id: 'database', label: 'Database', order: 4 }),
  helpWikiGroupSchema.parse({ id: 'development', label: 'Development', order: 5 }),
  helpWikiGroupSchema.parse({ id: 'history', label: 'History', order: 6 }),
  helpWikiGroupSchema.parse({ id: 'collaboration', label: 'Teams', order: 7 }),
  helpWikiGroupSchema.parse({ id: 'settings', label: 'Settings', order: 8 }),
  helpWikiGroupSchema.parse({ id: 'reference', label: 'Reference', order: 9 }),
] as const;
